...
/obj/mecha
	...
	var/datum/fluid_mixture/cabin_air
...
/obj/mecha/fire_act(datum/fluid_mixture/air, exposed_temperature, exposed_volume)
...
/obj/mecha/proc/return_pressure()
	. = 0
	if(use_internal_tank)
		. =  RETURN_PRESSURE(cabin_air)
	else
		var/datum/fluid_mixture/t_air = get_turf_air()
		if(t_air)
			. = RETURN_PRESSURE(t_air)
	return
...
/obj/mecha/proc/go_out()
	...
	/*
		if(ishuman(occupant) && (return_pressure() > HAZARD_HIGH_PRESSURE))
			use_internal_tank = 0
			var/datum/fluid_mixture/environment = get_turf_air()
			if(environment)
				var/env_pressure = RETURN_PRESSURE(environment)
				var/pressure_delta = (RETURN_PRESSURE(cabin) - env_pressure)
		//Can not have a pressure delta that would cause environment pressure > tank pressure
			...
			//Actually transfer the gas
					var/datum/fluid_mixture/removed = cabin.air_contents.remove(transfer_moles)
...
/datum/global_iterator/mecha_tank_give_air
	...
	process(var/obj/mecha/mecha)
		if(mecha.internal_tank)
			var/datum/fluid_mixture/tank_air = mecha.internal_tank.return_air()
			var/datum/fluid_mixture/cabin_air = mecha.cabin_air

			var/release_pressure = mecha.internal_tank_valve
			var/cabin_pressure = RETURN_PRESSURE(cabin_air)
			var/pressure_delta = min(release_pressure - cabin_pressure, (RETURN_PRESSURE(tank_air) - cabin_pressure)/2)
			var/transfer_moles = 0
			if(pressure_delta > 0) //cabin pressure lower than release pressure
				if(tank_air.temperature > 0)
					transfer_moles = pressure_delta*cabin_air.volume/(cabin_air.temperature * R_IDEAL_GAS_EQUATION)
					var/datum/fluid_mixture/removed = tank_air.remove(transfer_moles)
					cabin_air.merge(removed)
			else if(pressure_delta < 0) //cabin pressure higher than release pressure
				var/datum/fluid_mixture/t_air = mecha.get_turf_air()
				pressure_delta = cabin_pressure - release_pressure
				if(t_air)
					pressure_delta = min(cabin_pressure - RETURN_PRESSURE(t_air), pressure_delta)
				if(pressure_delta > 0) //if location pressure is lower than cabin pressure
					transfer_moles = pressure_delta*cabin_air.volume/(cabin_air.temperature * R_IDEAL_GAS_EQUATION)
					var/datum/fluid_mixture/removed = cabin_air.remove(transfer_moles)
					if(t_air)
						t_air.merge(removed)
	...
	process(var/obj/mecha/mecha)
		...
		if(mecha.hasInternalDamage(MECHA_INT_FIRE))
			...
			if(mecha.internal_tank)
				...
				var/datum/fluid_mixture/int_tank_air = mecha.internal_tank.return_air()
		...
		if(mecha.hasInternalDamage(MECHA_INT_TANK_BREACH)) //remove some air from internal tank
			if(mecha.internal_tank)
				var/datum/fluid_mixture/int_tank_air = mecha.internal_tank.return_air()
				var/datum/fluid_mixture/leaked_gas = int_tank_air.remove_ratio(0.10)
