...
/obj/structure/fitness/punchingbag/attack_hand(var/mob/living/carbon/human/user)
	if(!istype(user))
		..()
		return
	if(user.reagents.get_reagent_amount(/datum/reagent/hormone/glucose) <= GLUCOSE_LEVEL_LBAD)
		...
		if(user.a_intent == I_HURT)
			...
			user.reagents.remove_reagent(/datum/reagent/hormone/glucose, 0.1)
...
/obj/structure/fitness/weightlifter/attack_hand(var/mob/living/carbon/human/user)
	...
	if(being_used)
		...
		if(do_after(user, 20 + (weight * 10)))
			...
			user.reagents.remove_reagent(/datum/reagent/hormone/glucose, 0.1 * weight)
