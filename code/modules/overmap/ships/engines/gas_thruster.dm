...
/obj/machinery/atmospherics/unary/engine/proc/get_status()
	...
	.+= "Propellant pressure: [round(RETURN_PRESSURE(air_contents)/1000,0.1)] MPa."
	. = jointext(.,"<br>")
...
/obj/machinery/atmospherics/unary/engine/proc/calculate_thrust(datum/fluid_mixture/propellant, used_part = 1)
	return round(sqrt(propellant.get_mass() * used_part * RETURN_PRESSURE(air_contents)/100),0.1)
