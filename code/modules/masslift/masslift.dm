...
/datum/masslift/proc/transit(datum/cord/from)
	for(var/obj/effect/masslift_transit/T in masslift_transits)
		if(!T.busy)
			transit = T
			break
	get_area((C = from).to_turf()).move_contents_to(get_area(transit), with_gases = TRUE)

/datum/masslift/proc/from_transit(datum/cord/dest)
	transit.busy = FALSE
	get_area(transit).move_contents_to(get_area((C = dest).to_turf()), with_gases = TRUE)
	transit = null
...
/datum/masslift/proc/work()
	...
	if(istype(cable) && status == MASSLIFT_STATE_BUSY)
		...
		if(abs(depth - targets[1]) < 1)
			...
			from_transit(C.make_step(dir < 0 ? DOWN : UP, null))
			last = depth
	...
	if(get_zlevel() > 0 && status == MASSLIFT_STATE_BUSY)
		...
		if(depth > last)
			from_transit(C.make_step(DOWN, null))
		else if(depth < last)
			from_transit(C.make_step(UP, null))
