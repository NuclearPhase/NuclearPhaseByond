...
/atom/movable/openspace/multiplier/Destroy()
	var/turf/myturf = loc
	if (istype(myturf) && myturf.shadower == src)
...
/atom/movable/openspace/multiplier/proc/copy_lighting(atom/movable/lighting_overlay/LO)
	...
	if (icon_state == null)
		...
		icon_state = "white"	// this is actually just a white sprite, which is what this blending needs

		var/col = 204 - 204 * SHADOWER_DARKENING_FACTOR // 0xcc
