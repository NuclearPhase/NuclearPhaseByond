...
/obj/machinery/disposal
	...
	anchored = 1
	density = 1
	var/datum/fluid_mixture/air_contents	// internal reservoir
...
/obj/machinery/disposal/New()
	...
	spawn(5)
		...
		air_contents = new/datum/fluid_mixture(PRESSURE_TANK_VOLUME)
		update_icon()
...
/obj/machinery/disposal/interact(mob/user, var/ai=0)
	...
	var/per = 100* RETURN_PRESSURE(air_contents) / (SEND_PRESSURE)
...
/obj/machinery/disposal/Process()
	...
	if(flush && RETURN_PRESSURE(air_contents) >= SEND_PRESSURE )	// flush can happen even without power
		flush()

	if(mode != 1) //if off or ready, no need to charge
		update_use_power(1)
	else if(RETURN_PRESSURE(air_contents) >= SEND_PRESSURE)
...
/obj/structure/disposalholder
	invisibility = 101
	var/datum/fluid_mixture/gas = null	// gas used to flush, will appear at exit point
	...
	proc/init(var/obj/machinery/disposal/D, var/datum/fluid_mixture/flush_gas)
