...
proc/d2sm_prepmap(filename)
	...
	if(!txt) return
	var/i,j
	i=findtext_char(txt,ascii2text(13))	// eliminate carriage returns
	while(i)
		txt=copytext_char(txt,1,i)+copytext_char(txt,i+1)
		i=findtext_char(txt,ascii2text(13),i)
	i=findtext_char(txt,"\\\n")
	while(i)
		for(j=i+2,j<=length(txt),++j) if(text2ascii_char(txt,j)>32) break
...
proc/dmp2swapmap(filename)
	...
	/* i=findtext_char(txt,ascii2text(13))
	while(i)
		txt=copytext_char(txt,1,i)+copytext_char(txt,i+1)
		i=findtext_char(txt,ascii2text(13),i)
	i=findtext_char(txt,"\\\n")
	...
	while(txt)
		if(text2ascii_char(txt)==34)
			...
			k=findtext_char(txt,"\n",++i)
			j=(k || length(txt+1))
			while(--j>=i && text2ascii_char(txt,j)!=41)
			if(j<i)
		...
		else if(text2ascii_char(txt)==40)
			mode=40
			...
			if(findtext_char(mtxt,"\"\n")!=1 || !findtext_char(mtxt,"\n\"",length(mtxt)-1))
				to_world(findtext_char(mtxt,"\"\n"))
...
proc/d2sm_ParseCommaList(txt)
	...
	for(i=1,i<=length(txt),++i)
		if(text2ascii_char(txt,i)>32) break
	for(,i<=length(txt),++i)
		ch=text2ascii_char(txt,i)
		if(ch==44)
			L+=copytext_char(txt,1,i)
			for(++i,i<=length(txt),++i) if(text2ascii_char(txt,i)>32) break
