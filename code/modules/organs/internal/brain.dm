				...
				if(-(INFINITY) to BLOOD_PERFUSION_SURVIVE)
					if(!owner.is_floating)
						spawn()
							...
							sleep(30 SECONDS)
							owner.stop_floating()
					owner.eye_blurry = max(owner.eye_blurry, 20)
					owner.dizziness = max(owner.dizziness, 30)
					damprob = owner.chem_effects[CE_STABLE] ? 80 : 100
					if(prob(damprob))
						take_damage(1)
					if(!owner.paralysis && prob(20))
						owner.resting = TRUE
						to_chat(owner, "<span class='warning'>You feel falling..</span>")
	..()
