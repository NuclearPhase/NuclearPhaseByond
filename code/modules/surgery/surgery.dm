	...
	if(A)
		for (var/mob/living/carbon/human/H in A.contents)
			if (H.lying || germ_level >= 200)
				continue
			if (!(istype(H.head, /obj/item/clothing/head/helmet/space/rig/medical)))
			...
			if (!(istype(H.wear_suit, /obj/item/clothing/suit/space/rig/medical)))
	...
	for(var/datum/surgery_step/S in surgery_steps)
		...
		if(S.tool_quality(src))
			...
			if(step_is_valid && S.is_valid_target(M))
				...
				if(user.skillcheck(SKILL_ANATOMY, SKILL_AMATEUR, affected = user.intelligence))
