...
/obj/machinery/logic/indicator/bulb/process()
	...
	if(pn_input.available >= LOGIC_HIGH)
		icon_state = "bulb1"
	else
		icon_state = "bulb0"
...
/obj/machinery/logic/sensor/constant_high/process()
	...
	pn_output.newavail = max(pn_output.available, LOGIC_HIGH)
...
/obj/machinery/logic/oneinput/not/process()
	...
	if( !(pn_input.available >= LOGIC_HIGH))
		pn_output.newavail = max(pn_output.available, LOGIC_HIGH)	//Set the output avilable power to 5 or whatever it was before.
	else
...
/obj/machinery/logic/twoinput/and/process()
	...
	if( (pn_input1.available >= LOGIC_HIGH) && (pn_input2.available >= LOGIC_HIGH) )
		pn_output.newavail = max(pn_output.available, LOGIC_HIGH)	//Set the output avilable power to 5 or whatever it was before.
	else
		pn_output.draw_power(LOGIC_HIGH)		//Otherwise increase the load to 5
...
/obj/machinery/logic/twoinput/relay/process()
	...
	if( pn_input1.available >= LOGIC_HIGH )
		var/datum/powernet/pn_input2 = input2.powernet
		var/datum/powernet/pn_output = output.powernet
		...
		if(pn_input2.available >= RELAY_POWER_TRANSFER)
