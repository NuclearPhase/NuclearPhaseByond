...
/obj/machinery/power/sensor/proc/return_reading_text()
	...
	out += "<br><b>TOTAL AVAILABLE: [reading_to_text(powernet.available)]</b>"
	...
	out += "<br><b>TOTAL GRID LOAD: [reading_to_text(powernet.load())] ([round((powernet.load() / powernet.available) * 100)]%)</b>"
...
/obj/machinery/power/sensor/proc/return_reading_data()
	...
	data["total_avail"] = reading_to_text(max(powernet.available, 0))
	data["total_used_apc"] = reading_to_text(max(total_apc_load, 0))
	data["total_used_other"] = reading_to_text(max(powernet.load() - total_apc_load, 0))
	data["total_used_all"] = reading_to_text(max(powernet.load(), 0))
	// Prevents runtimes when available is 0 (division by zero)
	if(powernet.available)
		data["load_percentage"] = round((powernet.load() / powernet.available) * 100)
	else
