...
/obj/machinery/atmospherics/omni/filter/Process()
	...
	var/delta = clamp((output_air ? (max_output_pressure - RETURN_PRESSURE(output_air)) : 0), 0, max_output_pressure)
	...
	for(var/datum/omni_port/filter_output in gas_filters)
		delta = clamp((filter_output.air ? (max_output_pressure - RETURN_PRESSURE(filter_output.air)) : 0), 0, max_output_pressure)
