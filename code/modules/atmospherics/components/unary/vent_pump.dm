...
/obj/machinery/atmospherics/unary/vent_pump/proc/get_pressure_delta(datum/fluid_mixture/environment)
	...
	if(pump_direction) //internal -> external
		...
		if(pressure_checks & PRESSURE_CHECK_INTERNAL)
			pressure_delta = min(pressure_delta, RETURN_PRESSURE(air_contents) - internal_pressure_bound) //decreasing the pressure here
	else //external -> internal
		if(pressure_checks & PRESSURE_CHECK_EXTERNAL)
			pressure_delta = min(pressure_delta, environment_pressure - external_pressure_bound) //decreasing the pressure here
		if(pressure_checks & PRESSURE_CHECK_INTERNAL)
			pressure_delta = min(pressure_delta, internal_pressure_bound - RETURN_PRESSURE(air_contents)) //increasing the pressure here
