...
/datum/fluid_mixture/proc/update_values()
	UPDATE_VALUES(src)
...
/datum/fluid_mixture/proc/compare(const/datum/fluid_mixture/sample, var/vacuum_exception = 0)
	...
	if(abs(RETURN_PRESSURE(src) - RETURN_PRESSURE(sample)) > MINIMUM_PRESSURE_DIFFERENCE_TO_SUSPEND)
		return 0
