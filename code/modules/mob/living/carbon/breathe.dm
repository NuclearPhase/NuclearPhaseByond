...
/mob/living/carbon/proc/breathe(var/active_breathe = 1)
	...
	var/datum/fluid_mixture/breath = null
	...
	if(losebreath>0) //Suffocating so do not take a breath
		...
		if(!breath)
			var/static/datum/fluid_mixture/vacuum //avoid having to create a new gas mixture for each breath in space
...
/mob/living/carbon/proc/get_breath_from_environment(var/volume_needed=BREATH_VOLUME)
	...
	if(breath && breath.total_moles)
		...
		if(istype(wear_mask, /obj/item/clothing/mask) && breath)
			...
			var/datum/fluid_mixture/filtered = M.filter_air(breath)
			loc.assume_air(filtered)
		return breath
	return null

//Handle possble chem smoke effect
/mob/living/carbon/proc/handle_chemical_smoke(var/datum/fluid_mixture/environment)
	if(species && RETURN_PRESSURE(environment) < species.breath_pressure/5)
...
/mob/living/carbon/proc/handle_breath(datum/fluid_mixture/breath)
	return

/mob/living/carbon/proc/handle_post_breath(datum/fluid_mixture/breath)
	if(breath)
