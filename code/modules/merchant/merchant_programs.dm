...
/datum/computer_file/program/merchant/proc/transfer_to_bank()
	if(pad)
		var/list/targets = pad.get_targets()
		for(var/target in targets)
			if(istype(target, /obj/item/weapon/reagent_containers/food/snacks/protein_package))
				var/obj/item/weapon/reagent_containers/food/snacks/protein_package/cash = target
				bank += cash.worth
				qdel(target)
...
/datum/computer_file/program/merchant/proc/get_money()
	...
	var/obj/item/weapon/reagent_containers/food/snacks/protein_package/bundle/B = new(T)
	B.worth = bank
	bank = 0
	B.update_icon()
