...
#define LOCATE_COORDS(X, Y, Z) locate(clamp(X, 1, world.maxx), clamp(Y, 1, Z))
...
/area/proc/move_contents_to(var/area/A, with_gases = FALSE)
	//Takes: Area.
	//Returns: Nothing.
	...
	if(src_origin && trg_origin)
		...
		if(with_gases)
			translate_turfs_with_gases(translation, null)
		else
