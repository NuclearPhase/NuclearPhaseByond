...
//
/proc/translate_turfs_with_gases(var/list/translation, var/area/base_area = null, var/turf/base_turf)
	for(var/turf/source in translation)
		...
		if(target)
			...
			if(base_area)
				source.loc.contents.Add(target)
				base_area.contents.Add(source)
			transport_turf_contents_with_gases(source, target)
...
//
/proc/transport_turf_contents_with_gases(turf/source, turf/target)
